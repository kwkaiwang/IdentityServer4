...
namespace IdentityServer.IntegrationTests.Endpoints.Authorize
{
    public class JwtRequestAuthorizeTests
    {
        ...
        {
            var jwk = JsonSerializer.Deserialize<Microsoft.IdentityModel.Tokens.JsonWebKey>(_symmetricJwk); // new Microsoft.IdentityModel.Tokens.JsonWebKey(_symmetricJwk);
            var credential = new SigningCredentials(jwk, "HS256");
            var requestJwt = CreateRequestJwt(
                issuer: _client.ClientId,
                audience: IdentityServerPipeline.BaseUrl,
        ...
        public async Task authorize_should_accept_complex_objects_in_request_object()
        {
            ...
            var value = _mockPipeline.LoginRequest.RequestObjectValues["someObj"];
            ...
            var arrStr = _mockPipeline.LoginRequest.RequestObjectValues["someArr"];
            var arrValue = JsonSerializer.Deserialize<string[]>(arrStr);
